...
export function Legend({ states }: { states: ElementState[] }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 border-t border-line px-4 py-2">
      <span className="text-2xs font-medium text-fg-faint">Legend</span>
      {states.map((k) => (
        <span key={k} className="inline-flex items-center gap-1.5 text-2xs text-fg-muted" title={STATE_META[k].description}>
